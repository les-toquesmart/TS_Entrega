...
function renderStep(step: number, delivery: DeliveryProject, patch: (fn: (draft: DeliveryProject) => void) => void, finalize: () => void) {
  ...
  if (step === 3) return <><h1>Estado de entrega</h1><p className="subtitle">Registra pendientes y futuras etapas.</p><section className="stacked-section"><h2>Pendientes menores</h2><textarea value={delivery.state.minor} onChange={(e) => patch(d => { d.state.minor = e.target.value })}/><label><input type="checkbox" checked={delivery.state.minorNone} onChange={(e) => patch(d => { d.state.minorNone = e.target.checked })}/> No existen pendientes menores.</label></section><section className="stacked-section"><h2>Change orders adicionales</h2><p className="section-note">Trabajos agregados luego, fuera del alcance del proyecto original o de lo estipulado en este documento.</p><textarea value={delivery.state.phase2} onChange={(e) => patch(d => { d.state.phase2 = e.target.value })}/><label><input type="checkbox" checked={delivery.state.phase2None} onChange={(e) => patch(d => { d.state.phase2None = e.target.checked })}/> No existen change orders adicionales.</label></section><section className="stacked-section"><h2>Observaciones generales</h2><textarea value={delivery.state.notes} onChange={(e) => patch(d => { d.state.notes = e.target.value })}/></section></>
